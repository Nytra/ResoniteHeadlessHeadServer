...
public class MeshUpdateTracker
{
	...
	public static void MeshChanged(Asset asset)
	{
		...
		//owner.World.RunInUpdates(7, () =>
		//{
		//	if (assetToRenderers.TryGetValue(asset, out var renderers))
		//	{
...
public class MeshConnector : IMeshConnector
{
	...
	{
		//if ((IWorldElement)Asset.Owner is LocalMeshProvider) return;
		//if ((IWorldElement)Asset.Owner is WireMeshBase) return;
		//if (((IWorldElement)Asset.Owner).FindNearestParent<Slot>()?.Name == null) return;
		//if (((IWorldElement)Asset?.Owner)?.FindNearestParent<Slot>()?.FindParent(s => s.Name == "Local Assets") != null) return;
		...
		//Thundagun.QueuePacket(new ApplyChangesMeshConnector(this));
		//MeshUpdateTracker.MeshChanged(Asset);
		void cb()
		{
			...
			//var conn = MeshUpdateTracker.assetToConnector[Owner.Mesh.Asset];
			//conn.lastOnUpdated.Invoke(true);
		}
		...
		//});
		//lastOnUpdated = onUpdated;
		firstRender = false;
		//Engine.Current.MeshUpdated();
	}
}
